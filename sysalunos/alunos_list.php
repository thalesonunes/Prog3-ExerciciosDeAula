...
<div class="container">
    <nav class="navbar navbar-expand-lg navbar-light bg-light rounded">
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th scope="col">Dta. Nascimento</th>
            ...
            <tbody>
                <?php
                    $sql = "SELECT idalunos, nome, matricula, dt_nascimento, dt_cadastro FROM alunos ORDER BY nome";
                    $resp = mysqli_query($conexao_bd, $sql);
                    while($rows=mysqli_fetch_row($resp)){
                        ...
                        echo "<td>$rows[4]</td>";
                        echo "<td>
                                <a class='btn btn-primary' href='alunos_edit.php?idAlunos=$rows[0]' 
                                role='button'>Editar</a>&nbsp;
                                <a class='btn btn-danger'  href='javascript:excluirAlunos($rows[0])'
        ...
        <a class="btn btn-lg btn-primary" href="alunos_edit.php?idAlunos=0" 
        role="button">Novo Aluno</a>
</div>
    <script type="text/javascript">
        function excluirAlunos(idAlunos){
            Swal.fire({
                title: 'Deseja realmente exluir?',
                text: "Você deseja realmente excluir este aluno?",
                icon: 'warning',
                ...
                if (result.value) {
                    window.location.href = "alunos_excluir.php?idAlunos=" + idAlunos;
                }
            })
        }
    </script>
